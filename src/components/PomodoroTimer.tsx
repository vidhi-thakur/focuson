    ...
    <div className='pomodoro'>
      <div className='container'>
        ...
        <section className='countdownTimer'>
          <h2>{currTime.min > 9 ? currTime.min : `0${currTime.min}`}:{currTime.sec > 9 ? currTime.sec : `0${currTime.sec}`}</h2>
        </section>

        {/* start/stop timer CTA */}
        <section className='timerControls'>
          <Button disabled={isTimerOn} onClick={startTimer} variant="contained">Start</Button>
          <Button disabled={!isTimerOn} onClick={stopTimer} variant="contained">Stop</Button>
        </section>

        {/* click and change options */}
        <section className='timerActions'>
          <Button className='btn' startIcon={<UnfoldMoreIcon />} onClick={changeAction}>{OPTIONS[currActionIndex].name}</Button>
        </section>
      </div>
    </div>
  )
